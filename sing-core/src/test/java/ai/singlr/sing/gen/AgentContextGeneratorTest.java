...
class AgentContextGeneratorTest {
  ...
  private static SingYaml fullConfig() {
    ...
    var agent =
        new SingYaml.Agent(
            ...
            null,
            null);
    var ssh = new SingYaml.Ssh("dev", List.of("ssh-ed25519 AAAA... uday@mbp"));
  ...
  void includesRepositoriesSection() {
    var repos =
        List.of(
            new SingYaml.Repo("https://github.com/singlr/acme.git", "acme", "main"),
            new SingYaml.Repo("https://github.com/singlr/lib.git", "lib", null));
    var config =
        new SingYaml(
            "test",
            "Test",
    ...
    assertTrue(md.contains("singlr/acme.git"));
    assertTrue(md.contains("(branch: main)"));
    assertTrue(md.contains("`~/workspace/lib`"));
    assertFalse(md.contains("lib` ← https://github.com/singlr/lib.git (branch:"));
  }
  ...
  void environmentIncludesGitIdentity() {
    var config =
        new SingYaml(
            ...
            null,
            null,
            new SingYaml.Git("Singlr Eng", "eng@singlr.ai", "token", null),
            null,
            null,
            null,
            null,
    ...
    assertTrue(md.contains("Git identity: Singlr Eng <eng@singlr.ai>"));
  }
