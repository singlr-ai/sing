...
public final class Banner {
  ...
  /** Prints the sing ASCII wordmark. */
  public static void printBranding(PrintStream out, Ansi ansi) {
    out.println();
    out.println(ansi.string("  @|bold,cyan  ___ _                |@"));
    out.println(ansi.string("  @|bold,cyan / __(_)_ _  __ _      |@"));
    out.println(ansi.string("  @|bold,cyan \\__ \\ | ' \\/ _` |     |@"));
    out.println(ansi.string("  @|bold,cyan |___/_|_||_\\__, |     |@"));
    out.println(ansi.string("  @|bold,cyan            |___/      |@"));
  }
  ...
  public static String stepLine(int step, int total, String description, Ansi ansi) {
    return ansi.string("  @|bold [" + step + "/" + total + "]|@ " + description);
  }
  ...
  public static void printUnsupported(HostDetector.HostInfo info, PrintStream out, Ansi ansi) {
    out.println(
        ansi.string("  @|bold,red \u2717 Unsupported operating system:|@ " + info.osPrettyName()));
    out.println(ansi.string("    @|faint sing currently supports:|@ @|bold Ubuntu 24.04+|@"));
  }
  ...
  public static void printPrerequisites(
    ...
    out.println();
    out.println(ansi.string("  @|bold Checking prerequisites...|@"));
  ...
  public static void printMissingPrerequisites(
    ...
    out.println(ansi.string("  @|bold,red Missing commands:|@ " + commands));
    if (!packages.isEmpty()) {
      out.println(
          ansi.string("  @|faint Install with:|@ @|bold apt-get install -y " + packages + "|@"));
    }
    out.println();
  }
  ...
  public static String stepSkippedLine(int step, int total, String detail, Ansi ansi) {
    return ansi.string("  @|bold [" + step + "/" + total + "]|@ @|faint \u2192 " + detail + "|@");
  }
  ...
  public static void printProjectCreated(String name, String sshUser, PrintStream out, Ansi ansi) {
    ...
    out.println(ansi.string("  @|bold,green \u2713 Project " + name + " created.|@"));
  ...
  public static void printSshConfig(
      ...
      Ansi ansi) {
    out.println();
    out.println(ansi.string("    @|faint # Add to ~/.ssh/config on your Mac:|@"));
    out.println();
    out.println(ansi.string("    @|faint Host singular-server|@"));
    out.println(ansi.string("    @|faint     HostName " + serverIp + "|@"));
    out.println(ansi.string("    @|faint     User " + serverUser + "|@"));
    out.println(ansi.string("    @|faint     IdentityFile ~/.ssh/id_ed25519|@"));
    out.println();
    out.println(ansi.string("    @|faint Host " + name + "|@"));
    out.println(ansi.string("    @|faint     HostName " + containerIp + "|@"));
    out.println(ansi.string("    @|faint     User dev|@"));
    out.println(ansi.string("    @|faint     ProxyJump singular-server|@"));
    out.println(ansi.string("    @|faint     IdentityFile ~/.ssh/id_ed25519|@"));
    out.println();
    out.println(
        ansi.string("    @|bold \u2192 Zed:|@     zed ssh://dev@" + name + "/home/dev/workspace"));
    out.println(ansi.string("    @|bold \u2192 SSH:|@     ssh " + name));
    out.println(ansi.string("    @|bold \u2192 Shell:|@   sudo sing shell " + name));
  }
  ...
  public static void printResumeInfo(ProvisionState state, PrintStream out, Ansi ansi) {
    out.println();
    out.println(ansi.string("  @|yellow Resuming incomplete provisioning|@"));
    if (state.completedPhase() != null) {
      out.println(ansi.string("    @|faint Last completed:|@ " + state.completedPhase()));
    }
    if (state.error() != null) {
      out.println(
          ansi.string(
  ...
  public static void printZedConnect(String name, String sshUser, PrintStream out, Ansi ansi) {
    out.println(ansi.string("    @|bold \u2192 Connect:|@ sing connect " + name));
    out.println(ansi.string("    @|bold \u2192 Shell:|@   sudo sing shell " + name));
  }
  ...
  public static void printAgentAuthTunnel(String name, PrintStream out, Ansi ansi) {
    out.println(
        ansi.string("    @|bold \u2192 Agent auth:|@ ssh -N -L 3000:localhost:3000 " + name));
  }
  ...
  public static void printContainerStatus(
    ...
    if (state instanceof ContainerState.Running r) {
      ...
      out.println(ansi.string("  @|green \u2713|@ " + name + " is running" + ip));
    } else if (state instanceof ContainerState.Stopped) {
      out.println(ansi.string("  @|faint \u25a0 " + name + " is stopped|@"));
    } else if (state instanceof ContainerState.NotCreated) {
      out.println(ansi.string("  @|faint \u25a0 " + name + " does not exist|@"));
    } else if (state instanceof ContainerState.Error e) {
      out.println(ansi.string("  @|bold,red \u2717|@ " + name + ": " + e.message()));
    }
  }
  ...
  public static void printProjectDestroyed(String name, PrintStream out, Ansi ansi) {
    out.println(ansi.string("  @|bold,green \u2713 Project " + name + " destroyed.|@"));
  }
  ...
  public static void printSnapshotCreated(
    ...
    out.println(ansi.string("  @|bold,green \u2713 Snapshot created:|@ " + label));
  }
  ...
  public static void printSnapshotRestored(
    ...
    out.println(
        ansi.string(
            "  @|bold,green \u2713 Restored|@ " + containerName + " to snapshot: " + label));
  }
  ...
  public static void printOvercommitWarning(
    ...
    out.println(ansi.string(sb.toString()));
    out.println(
        ansi.string("    @|faint Use 'sing down <name>' to stop projects and free resources.|@"));
  }
  ...
  public static void printAgentStatus(
    ...
    out.println(
        ansi.string(
            "  @|bold Agent session for "
                ...
                + statusStr
                + "|@"));
    out.println(ansi.string("    @|bold PID:|@        " + info.pid()));
    if (!info.task().isBlank()) {
      var taskPreview =
          info.task().length() > 60 ? info.task().substring(0, 60) + "..." : info.task();
      out.println(ansi.string("    @|bold Task:|@       " + taskPreview));
    }
    if (!info.startedAt().isBlank()) {
      out.println(ansi.string("    @|bold Started:|@    " + formatTimestamp(info.startedAt())));
    }
    if (!info.branch().isBlank()) {
      out.println(ansi.string("    @|bold Branch:|@     " + info.branch()));
    }
    if (commitCount > 0) {
      out.println(ansi.string("    @|bold Commits:|@    " + commitCount + " since launch"));
    }
    if (lastCommitMinutesAgo >= 0) {
      out.println(
          ansi.string("    @|bold Last commit:|@ " + lastCommitMinutesAgo + " minutes ago"));
    }
    if (taskCounts != null) {
    ...
    out.println(ansi.string("    @|bold Log:|@        " + info.logPath()));
    if (info.running()) {
      out.println(ansi.string("    @|faint Tail output: sing agent log " + name + " --follow|@"));
    }
  }
  ...
  public static void printAgentLaunched(
    ...
    out.println(ansi.string("  @|green \u2713|@ Agent launched in background"));
    if (task != null) {
      var taskPreview = task.length() > 60 ? task.substring(0, 60) + "..." : task;
      out.println(ansi.string("    @|bold Task:|@    " + taskPreview));
    }
    if (branch != null && !branch.isBlank()) {
      out.println(ansi.string("    @|bold Branch:|@  " + branch));
    }
    out.println(ansi.string("    @|bold Log:|@     sing agent log " + name + " --follow"));
    out.println(ansi.string("    @|bold Stop:|@    sing agent stop " + name));
  }
  ...
  public static void printAlsoRunning(
    ...
    out.println();
    out.println(ansi.string("  @|faint Also running:|@"));
    for (var c : others) {
      var ip = c.state() instanceof ContainerState.Running r && r.ipv4() != null ? r.ipv4() : "";
      var res = "";
      if (c.limits() != null) {
      ...
      out.println(ansi.string("    @|faint \u2022 " + c.name() + " " + ip + res + "|@"));
    }
  }
  ...
  public static void printAgentReport(
    ...
    out.println();
    out.println(ansi.string("  @|bold Agent Report \u2014 " + name + "|@"));
    out.println(
        ansi.string("  @|faint " + "\u2500".repeat(Math.max(20, 16 + name.length())) + "|@"));
    out.println();
    ...
    out.println(
        ansi.string(
            "  @|bold Session:|@    @|" + statusStyle + " " + report.sessionStatus() + "|@"));
    if (report.duration() != null) {
      out.println(ansi.string("  @|bold Duration:|@   " + report.duration()));
    }
    if (report.branch() != null && !report.branch().isBlank()) {
      out.println(ansi.string("  @|bold Branch:|@     " + report.branch()));
    }
    out.println();

    if (!report.specs().isEmpty()) {
      out.println(ansi.string("  @|bold Specs:|@"));
      for (var spec : report.specs()) {
        ...
        out.println(ansi.string("    " + icon + " " + idPad + spec.title()));
    ...
    out.println(ansi.string("  @|bold Git Activity:|@"));
    if (report.commitCount() > 0) {
      ...
      out.println(ansi.string(commitStr));
    } else {
      out.println(ansi.string("    @|faint No commits found.|@"));
    }
    out.println();
    ...
    if (report.guardrailTriggered()) {
      out.println(
          ansi.string(
              "  @|bold,yellow Guardrails:|@  Triggered (" + report.guardrailReason() + ")"));
      out.println(ansi.string("    @|faint Action: " + report.guardrailAction() + "|@"));
    } else {
      out.println(ansi.string("  @|bold Guardrails:|@  @|green Not triggered|@"));
    }

    if (report.rolledBack()) {
      out.println(
          ansi.string(
              "  @|bold,red Rollback:|@   Snapshot restored: " + report.rollbackSnapshot()));
    }
    out.println();

    out.println(ansi.string("  @|bold Next Steps:|@"));
    out.println(
        ansi.string("    sing agent log " + name + " --tail 50    @|faint # see agent output|@"));
    var remaining =
        report.specs().stream()
  ...
  private static void emptyRow(PrintStream out, Ansi ansi, int width) {
    out.println(ansi.string("  @|bold,cyan \u2502|@" + " ".repeat(width) + "@|bold,cyan \u2502|@"));
  }
