...
public final class ProjectResourcesSetCommand implements Runnable {
  ...
  private void execute() throws Exception {
    ...
    var desiredResources = SingYamlUpdater.mergeResources(config.resources(), cpu, memory, disk);
    ...
    if (descriptorChanged) {
      if (dryRun) {
        ...
        SingYamlUpdater.updateResources(singYamlPath, cpu, memory, disk);
      }
    }
  ...
  private static boolean limitsDiffer(ResourceLimits limits, SingYaml.Resources desired) {
    if (limits == null) {
      return true;
    }
  ...
  private void printAlreadyCurrent(Path singYamlPath, SingYaml.Resources desiredResources) {
    if (json) {
  ...
  private void printResult(
      Path singYamlPath,
      SingYaml.Resources previous,
      SingYaml.Resources desired,
  ...
  private static LinkedHashMap<String, Object> resourceMap(SingYaml.Resources resources) {
    var map = new LinkedHashMap<String, Object>();
