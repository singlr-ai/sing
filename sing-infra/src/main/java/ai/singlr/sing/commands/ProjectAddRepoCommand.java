...
public final class ProjectAddRepoCommand implements Runnable {
  ...
  private void execute() throws Exception {
    ...
    if (url != null) {
      ...
      repo = new SingYaml.Repo(url, path, branch);
    } else {
      if (!json) {
    ...
    SingYamlUpdater.addRepo(singYamlPath, repo);
  ...
  private SingYaml.Repo collectRepoInteractively(java.io.PrintStream out, Ansi ansi) {
    ...
    return new SingYaml.Repo(repoUrl, path, branchInput.isEmpty() ? null : branchInput);
  }
