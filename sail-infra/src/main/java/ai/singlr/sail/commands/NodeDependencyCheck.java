...
final class NodeDependencyCheck {
  ...
  sealed interface Resolution {
    record Unchanged(SailYaml config) implements Resolution {}

    record NodeAdded(SailYaml config) implements Resolution {}

    record AgentsDropped(SailYaml config, List<AgentCli> dropped) implements Resolution {}
  ...
  static List<AgentCli> findNodeDependentAgents(SailYaml config) {
  ...
  static boolean hasNodeRuntime(SailYaml config) {
    return config.runtimes() != null && config.runtimes().node() != null;
  }

  static Resolution resolve(SailYaml config, boolean autoAccept) {
  ...
  static void failNonInteractive(SailYaml config) {
    var nodeAgents = findNodeDependentAgents(config);
    if (nodeAgents.isEmpty() || hasNodeRuntime(config)) {
      return;
    }
  ...
  private static List<String> resolveInstallList(SailYaml config) {
    if (config.agent() == null) {
      return List.of();
    }
